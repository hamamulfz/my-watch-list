...
class SslHelper {
  static Future<http.Client> createEnvironmentClient() async {
    IOClient client = IOClient(await SslHelper.customeHttpClient());
    return client;
  }

  static Future<HttpClient> customeHttpClient() async {
    final context = SecurityContext(withTrustedRoots: false);

    try {
      List<int> certFileBytes = [];
      try {
        certFileBytes = (await rootBundle.load('assets/certs/certificates.pem'))
            .buffer
            .asInt8List();
